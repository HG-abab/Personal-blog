<script setup>
  ...
  async function refreshFunc() {
    loading.value = true;
    // 使用模拟数据
    tabData.value = mockFavoriteData.value.map((item) => {
  ...
  async function onFinish(values) {
    ...
    // 模拟搜索功能
    const filteredData = mockFavoriteData.value.filter((item) => {
      const matchUserName = submitData.userName ? item.userName.includes(submitData.userName) : true;
      const matchType = submitData.type !== undefined ? item.type === submitData.type : true;
      const matchIsCheck = submitData.isCheck !== undefined ? item.isCheck === (submitData.isCheck === 1) : true;
      return matchUserName && matchType && matchIsCheck;
    });
  ...
  function updateIsCheckFunc(id, isCheck, record) {
    ...
    // 模拟更新操作
    setTimeout(() => {
      const index = mockFavoriteData.value.findIndex((item) => item.id === id);
      if (index !== -1) {
        mockFavoriteData.value[index].isCheck = isCheck === 1;
      }
  ...
  function onDelete(ids) {
    if (ids) {
      // 模拟删除操作
      mockFavoriteData.value = mockFavoriteData.value.filter((item) => !ids.includes(item.id));
