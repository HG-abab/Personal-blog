class TagCloud {
  constructor(container = document.body, texts, options) {
    ...
    //将配置对象中的 radius 属性值赋给实例属性 self.radius，这个属性决定了标签云滚动的半径
    self.depth = 2 * self.radius
    //计算 depth 属性，通常用于3D效果，这里设置为滚动半径的两倍
    self.size = 1.5 * self.radius
    //计算 size 属性，它可能用于确定鼠标交互区域的大小，这里设置为滚动半径的1.5倍。
    self.maxSpeed = TagCloud._getMaxSpeed(self.config.maxSpeed)
    // 调用静态方法 _getMaxSpeed 来获取最大滚动速度，并将结果赋给 self.maxSpeed。这个方法可能根据传入的 maxSpeed 配置值返回一个计算后的值。
    self.initSpeed = TagCloud._getInitSpeed(self.config.initSpeed)
    // 调用静态方法 _getMaxSpeed 来获取初始滚动速度，并将结果赋给 self.maxSpeed。这个方法可能根据传入的 maxSpeed 配置值返回一个计算后的值。
    self.direction = self.config.direction
    // 将配置对象中的 direction 属性值赋给实例属性 self.direction，这个属性决定了标签云滚动的初始方向
    self.keep = self.config.keep
    // 将配置对象中的 keep 属性值赋给实例属性 self.keep，这个属性决定了鼠标离开标签云区域后是否继续滚动。
  ...
  _createTextItem(text, index = 0) {
    ...
    if (self.config.useItemInlineStyles) {
      ...
      itemEl.style.color = self._randomRgb()
